# 01 — The stride-16 layout and the `shr` primitive

*A paper on `Bitboard::shr` in the Gomoku engine (slice 3,
`gomoku/crates/engine/src/bitboard.rs`). Written to be read top-down:
goal → representation → the construction → why a shift is the primitive
→ mechanism → the invariants it depends on → contract → tests.
Terminology follows the [tutorial glossary](../README.md#glossary).*
...
worker threads, while a neural network does the expensive computation.
Two requirements follow:

1. **"Is this position decided?" must be nearly free.** If the rules
   engine is not negligible next to the network, self-play throughput
   drops.
...
The reference engine (slice 2) answers by walking: from a stone, look at
the four axes and count neighbours. Simple, obviously correct, and full
of per-cell bounds checks and unpredictable branches. It is the
*walk-based oracle* — it stays as the thing the fast version must agree
with.

The design goal for `has_five` is therefore: express "are there five
stones in a line?" as branchless bit algebra over a handful of 64-bit
words.

## 2. The representation this requires

Bit algebra needs cells to be *bits*, at linearly addressable, uniformly
strided indices. Hence the stride-16 layout:
...
| `w[3]` | 192–255 | rows 12–14 (bits 240–255 unused) |
...
```text
 bit:  15  14  13  12  ...   2   1   0
        ...
        |
        padding bit (column 15) — always zero
```

**The stride is 16, the board is 15 wide.** The padding bit per row is
not a rounding artifact; it is the mechanism that makes the shift
construction correct (§6). Everything else in this paper follows from
it.

## 3. Five-in-a-row as AND of shifted copies
...
AND keeps `j` only if *both* `j` and `j+s` are stones. Repeating the
construction counts *runs*: 2 → 4 → 5.
...
Each axis is covered once; the sign of the direction does not matter for
"is there a line of five".

### The naive five-term AND

The direct expression is

```rust
// DO NOT USE: shifts up to 4·17 = 68 — beyond the 64-bit range
...
A consequence the rules require anyway: an **overline counts** (a run of
six also satisfies "run of ≥ 5"), which is chapter 13's decision 1 — at
no extra cost.
...
This is the complete specification — everything mechanical follows from
it:
...
Note that it is a *downward* shift in index space. On the board that
means "look at the cell one step up the index axis" — which is what
`b & b.shr(s)` needs, because the run is then detected at its **lowest**
index (and `!five.is_zero()` does not care where).
...
Two of those cross a 16-bit row slot, and the first one crosses a
**word** boundary (`w[1]` → `w[0]`) — see §5 for why that needs care.
...
The carry term is the only subtle line:
...
- Those lost bits are fine (they belong below the word), but word `k`'s
  ...
  **low `s` bits** — which is exactly `w[k+1] << (64 - s)`: shifting
  left by `64 - s` moves the low `s` bits up to the top.
- `w[3] >> s` has no carry because there is nothing above the 256th
  bit; zeros entering the top is correct.
...
masked-amount garbage in release. Likewise `s >= 64`. The design guards
this:
...
The staged AND never needs more than `2 · 17 = 34`.
...
type, documents `0 < s < 64` next to the signature, and is not part of
the public API (`pub(crate)`) — `shr` is an implementation detail of
the engine, not a service offered to other crates.

## 6. Why the padding column makes this correct

The construction has one failure mode: **wraps**. A shift does not know
about rows. `(r, 14) + 1` is row `r`'s padding bit, not `(r+1, 0)`, and
`(r, 0) + 15` is row `r`'s padding bit, not `(r-1, 1)`. Without a zero
bit at those positions, a chain could cross a row boundary and produce
a phantom five.
...
`(1,0)` — which would be a five if the chain could cross rows — is
correctly **not** a five.

### The sanitising property (why an intermediate shift may be dirty)

A shift *can* deposit a real stone onto a padding bit: `(1,14)` is index
30, and `30 - 15 = 15` = `(0,15)`, the padding bit. Verified:

```text
cells(b)             = {(1,14)}
cells(b.shr(15))     = {(0,15)}     ← lands on a padding bit, "dirty"
cells(b & b.shr(15))= {}            ← the AND removes it
```

The reason this always works is an induction:

- `b` (and any board living in the engine) has **zero padding bits**.
- Every intermediate is `X & X.shr(s)` for some `X` that already has
  zero padding bits. For the AND to set padding bit `g`, `X` would need
  bit `g` set — it does not. So `two`, `four`, `five` all have zero
  padding bits.
- Therefore the *un-shifted* operand in each AND re-establishes the
  invariant at every stage. **The shifts may be geometrically dirty;
  the ANDs enforce the geometry.**

### Two kinds of padding, and why `& VALID` is mandatory after `!`

| padding | count | purpose |
|---|---|---|
| padding column (column 15 of every row) | 15 | geometry: stop wraps |
| high padding (bits 240–255) | 16 | hygiene: nothing real lives there |

`256 − 31 = 225` real cells. Both kinds must stay zero, and the reason
is the complement: `!occupied` sets *all* padding bits to one. Set
padding-column bits would recreate exactly the phantom fives of the
table above, and a set bit at 240–255 would be dragged *down* into real
cells by the next right shift. Hence the rule from the design:
...
`VALID` is the mask of all 225 real cells: `0x7fff` per row slot, and
`0x0000_7fff_7fff_7fff` for the tail word. The padding column is bit
15 — the MSB of the slot — because `idx = r*16 + c` numbers columns
...
- **no branches and no bounds checks** inside a direction, and
  `DIRS.iter().any(...)` short-circuits on the first hit

The walk-based oracle does fewer *logical* operations but pays up to 32
bounds-checked loads and unpredictable branches per call. Branchless
uniformity is the source of the speedup, and slice 9's criterion
benchmark is what verifies it (acceptance: `has_five` ≥ 50M checks/s).
...
| row-slot crossing | `(1,1)` shr 1 → `(1,0)`; `(3,14)` shr 1 → padding bit → empty |
...
**Caveat.** Hand-picked `shr` tests should not use `(0,0)` as the
*source* stone: under `shr` a stone at index 0 is at the bottom of the
address space, so all four results are `EMPTY` (verified) — the
assertion passes for the wrong reason. Use `(1,1)` and the
word-boundary case from the table above instead; a stone at `(0,0)` is
only interesting as the *target* of a shift.
...
"is the position decided?" must be ~free in the self-play hot loop
  ...
  → a run is "AND of a copy shifted by the direction constant s"
  ...
  → shifts can cross rows  → stride 16's padding bit per row stops wraps
  → complement sets padding      → every complement ANDs with VALID
  → s = 0 or s ≥ 64 breaks `<< (64-s)`  → debug_assert!(0 < s < 64)
```

Summary table — "what you want" → "what you write":
...
- **Symmetry (D4)** is planned as const-generated index-permutation
  tables (slice 6, `[[u8; 225]; 8]`). Rotations and reflections are not
  uniform strides, so shifts are the wrong primitive there.
- **Tactics** (slice 7) does not need its own shifts: `immediate_wins`
  and `double_threats` reuse the `has_five` primitive with a
  hypothetical stone placed — which is why `has_five` must be a pure
  function of a `Bitboard`, not of the move history.
- **Encoding** (slice 6) works on `u8` planes, not bitboards.

`shr` has exactly one reason to exist — win detection — and it serves
the tactic queries that are built on top of `has_five`.
