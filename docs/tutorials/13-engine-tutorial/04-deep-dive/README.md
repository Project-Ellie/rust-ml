...
| # | Paper | Answers |
|---|-------|---------|
| 01 | [The staged AND, and why win detection needs no edge masks](01-staged-and-and-no-edge-masks.md) | Why two hops of 2 and 4 instead of one five-term AND — and why the naive form answers incorrectly for two of the four directions; why the padding invariant removes every per-direction edge mask (539 phantom fives without it, 0 with it); where a mask *is* required; what "≥ 5" costs if you ever count with it; which of the two call shapes belongs to `Board::play` and which to MCTS; and how to benchmark bit twiddling without measuring the optimizer. |
