# Chapter 02 — Deep-dive solution: the arena

This is the opt-in reference for Chapter 02. `tree.rs` is quoted
...
pub struct Edge {
    ...
    pub mv: engine::Move,
    /// Prior probability `P(s, a)` assigned when the parent was expanded.
    pub prior: f32,
    /// Visit count `N(s, a)`.
    pub n: u32,
    ...
    pub w: f32,
    /// Child node index, created eagerly during expansion (primer §6.3
    /// note: the arena keeps children contiguous; `Option` stays in the
    /// API for future lazy-expansion experiments).
...
pub enum NodeState {
    ...
    Expanded,
    /// Game-over node: no edges, exact value known.
    Terminal,
}
...
impl Node {
    ...
    /// Mutable lifecycle state (used by expansion).
    ...
    /// Mutable outgoing edges (used by backup and child attachment).
    #[must_use]
...
impl Tree {
    /// Create a tree containing a single unexpanded root node (id 0).
    #[must_use]
    pub fn new() -> Self {
        Self {
            nodes: vec![Node::new()],
    ...
    ///
    /// # Panics
    /// Panics if `id` is out of bounds — a bug in every call site.
    #[must_use]
...
mod tests {
    ...
    fn arena_grows_and_indices_stay_stable() {
        ...
        // Create two edges at the root first, then attach children.
        ...
        // A child can have edges pre-created before its own children:
        ...
        assert_eq!(b, 2);
        assert_eq!(a0, 3);
        // Indices remain stable after further growth.
